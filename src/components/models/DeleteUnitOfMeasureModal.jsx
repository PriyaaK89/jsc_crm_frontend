...
const DeleteUnitOfMeasureModal = ({ selectedId, deleteModelIsOpen, deleteModelOnClose, getUnitList }) => {
    ...
    const handleDeleteUnit  = async()=>{
        try{
              const response = await API?.delete(`${API_ENDPOINTS?.delete_unitOfMeasure}/${selectedId}`);

              if(response?.status === 200){
                toast({
                    description: 'Unit is deleted successfully...',
                    duration: 1000,
                    status: 'success',
                    isClosable: true
                })
            ...
            <ModalContent mx="12px" borderRadius="12px">
                ...
                <ModalBody mt={4}>
                    <Text fontSize={{ base: "12px", md: "14px" }}>
                        ...
                        <b style={{ color: "red" }}>delete</b> this Unit?
                        <br />
...
export default DeleteUnitOfMeasureModal
