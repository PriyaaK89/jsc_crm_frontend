...
const ReceiptInvoice = ({ isOpen, onClose, receiptId }) => {
    const [receiptDetails, setReceiptDetails] = useState(null);
    const [loading, setLoading] = useState(false);
    const [downloading, setDownloading] = useState(false);
    const invoiceRef = useRef(null);

    const getReceiptInvoiceDetails = async () => {
        if (!receiptId) return;
        try {
            setLoading(true);
            const response = await API.get(
                `${API_ENDPOINTS.GET_RECEIPT_INVOICE}/${receiptId}`
            );
            if (response.status === 200) {
                setReceiptDetails(response.data.data);
            }
        } catch (error) {
            console.log(error, "Error in fetching Receipt Invoice Details");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownloadPdf = async () => {
        if (!invoiceRef.current || !receiptDetails) return;
        try {
            setDownloading(true);
            const html2pdf = (await import("html2pdf.js")).default;
            const options = {
                margin: 5,
                filename: `Receipt_Voucher_${receiptDetails.receipt.receipt_no || receiptId
                    }.pdf`,
    ...
    const renderInvoiceContent = () => {
        ...
        const { receipt, entries, billReferences } = receiptDetails;
        const amountInWords = `Rupees ${toWords(
            Number(receipt.total_amount)
        )} Only`;
        const formattedDate = new Date(receipt.receipt_date).toLocaleDateString(
            "en-GB"
        );
        ...
        return (
            ...
            >
                ...
                {/* ===== VOUCHER DETAILS ===== */}
                <VStack  gap={0.5} alignItems="baseline" mb={1}>
                    <Text fontFamily="serif" fontSize="11px" fontWeight="900"> Order No. : {receipt.id || "-"} </Text>
                    <Text fontFamily="serif" fontSize="11px" fontWeight="900"> Receipt No. : {receipt.voucher_no || "-"} </Text>
                    <Text fontFamily="serif" fontSize="11px" fontWeight="900"> Date : {formattedDate} </Text>
                    <Text fontFamily="serif" fontSize="11px" fontWeight="900"> Under Emp. : {receipt.employee_under_name || "-"} </Text>
                    {/* <Text fontSize="11px" fontWeight="900"> Party Id. : JSC-P{receipt.account_gst_no || "-"} </Text> */}
                    <Text fontFamily="serif" fontSize="11px" fontWeight="900">
                        {entries.map((item)=>(<>Party Id. : {`JSC-P-${item?.ledger_id}`}</>))}
                    </Text>
                </VStack>
                ...
                <Box border="1px solid #000" mt={3} p={3} fontSize="8px">
                    ...
                    <Text mt="2px" fontFamily="serif" color="black">
                        ❖ Distributor has to send notice to company within 7 days if any complaint regarding the product or anything, otherwise it will be discarded.
                    </Text>
                    <Text mt="2px" fontFamily="serif" color="black">
                ...
                <ModalHeader borderBottom="1px solid #e2e8f0" py={3}>
                    <Flex justify="space-between" align="center" pr={8}>
                        ...
                        <Button
                            ...
                            isDisabled={!receiptDetails || loading}
