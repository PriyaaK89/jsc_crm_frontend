...
const DebitNoteInvoice = ({ isOpen, onClose, debitNoteId }) => {
  ...
  const fetchInvoice = async () => {
    ...
    try {
      ...
      const response = await API.get(
        `${API_ENDPOINTS.GET_DEBIT_NOTE_INVOICE}/${debitNoteId}`
      );
      if (response.status === 200) {
        setInvoice(response.data.data);
      }
    } catch (error) {
      console.log("Error fetching debit note invoice:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const options = {
        margin: 5,
        filename: `Debit_Note_${invoice?.debitNote?.voucher_no || debitNoteId}.pdf`,
  ...
  const renderInvoiceContent = () => {
    ...
    const grossTotal = totalAmount + totalTaxAmount;
    const finalTotal = Math.round(grossTotal);
    const roundOff = finalTotal - grossTotal;

    // ── GST Totals from items (since debit_notes may not have cgst_total) ───
    ...
    const igstTotal = items.reduce(
      (sum, item) => sum + parseFloat(item.igst_amount || 0), 0
    );
    ...
    return (
      ...
      >
        ...
        <Table size="sm" variant="unstyled" mb={1}>
          <Thead>
            <Tr>
              <Th fontSize="10px">Supplier (Bill To):</Th>
          ...
          <Tbody>
            <Tr>
              ...
              <Td verticalAlign="top" width="35%">
                ...
                <Text fontFamily="serif" color="black" fontSize="10px">
                  Contact No.: {dn.supplier_mobile || "-"}
                </Text>
                <Text fontFamily="serif" color="black" fontSize="10px">
                  GSTN No.: {dn.supplier_gst || "-"}
                </Text>
                <Text fontFamily="serif" color="black" fontSize="10px">
              ...
              <Td verticalAlign="top" width="35%">
                <Text fontFamily="serif" color="black" fontSize="10px">
                  Transport Name: By {dn.transport_name || "-"}
                </Text>
                <Text fontFamily="serif" color="black" fontSize="10px">
                ...
                <Text fontFamily="serif" color="black" fontSize="10px">
                  Delivery Loc.: {dn.delivery_place || "-"}
                </Text>
              </Td>
              ...
              <Td verticalAlign="top" width="30%">
                <Text fontFamily="serif" color="black" fontSize="10px" fontWeight="900">
                  Under Emp.: {dn.employee_under_name || "-"}
                </Text>
                <Text fontFamily="serif" color="black" fontSize="10px" fontWeight="900">
                  Assigned Emp.: {dn.assign_employee_name || "-"}
                </Text>
                <Text fontFamily="serif" color="black" fontSize="10px" fontWeight="900">
                  Order No.: {dn.order_no || "-"}
                </Text>
                <Text fontFamily="serif" color="black" fontSize="10px" fontWeight="900">
                  Order Date:{" "}
                  {dn.debit_note_date
              ...
              {items.map((item, index) => {
                ...
                return (
                  <Tr key={item.id} borderBottom="1px solid #ccc">
                    ...
                    <Td textAlign="center">
                      {parseFloat(item.return_qty || 0).toFixed(0)}
                    </Td>
                    <Td textAlign="center">
        ...
        <Flex justify="flex-end" mt={2} pb={1}>
          <Box minW="260px">
            ...
            {Math.abs(roundOff) > 0 && (
              <Flex justify="space-between">
                <Text fontSize="12px" fontWeight="600">Round Off :</Text>
                <Text fontSize="12px" fontWeight="600">
                  {roundOff.toFixed(2)}
                </Text>
              </Flex>
        ...
        <Flex gap={2} py={1} borderBottom="1px solid #4e4e4e" fontSize="11px">
          ...
          <Text>{dn.narration || "-"}</Text>
        </Flex>

        {/* ===== BILL REFERENCES (if any) ===== */}
        {billReferences.length > 0 && (
          <Box mt={2} mb={2}>
            <Text fontFamily="serif" color="black" fontWeight="bold" fontSize="11px" mb={1}>
              Bill References :
            </Text>
            <Table size="sm" variant="unstyled" sx={{ borderCollapse: "collapse" }}>
              <Thead>
                <Tr borderTop="1px solid #4e4e4e" borderBottom="1px solid #4e4e4e">
                  <Th fontSize="10px">Ref. No.</Th>
                  <Th fontSize="10px">Bill Amount</Th>
                  <Th fontSize="10px">Pending Amount</Th>
                  <Th fontSize="10px">Due Date</Th>
                </Tr>
              </Thead>
              <Tbody>
                {billReferences.map((ref) => (
                  <Tr key={ref.id} borderBottom="1px solid #ccc">
                    <Td fontSize="10px">{ref.reference_no || "-"}</Td>
                    <Td fontSize="10px" textAlign="left">
                      {parseFloat(ref.bill_amount || 0).toFixed(2)}
                    </Td>
                    <Td fontSize="10px" textAlign="left">
                      {parseFloat(ref.pending_amount || 0).toFixed(2)}
                    </Td>
                    <Td fontSize="10px">
                      {ref.due_date
      ...
      <ModalContent maxW="960px">
        ...
        <ModalHeader borderBottom="1px solid #e2e8f0" py={3}>
          <Flex justify="space-between" align="center" pr={8}>
            <Text fontSize="16px" fontWeight="600">Debit Note Invoice</Text>
            <Button
              colorScheme="green"
              size="sm"
              fontSize="12px"
